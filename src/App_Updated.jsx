...
function App() {
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      const randomRecipes = await fetchRandomRecipes(10);
  ...
  const searchRecipes = useCallback(async () => {
    ...
    try {
      ...
      // If diet filter is active, add it as category
      if (activeFilter !== 'All' && dietFilters.includes(activeFilter)) {
        searchParams.category = activeFilter;
      }
      
      const searchResults = await fetchRecipesFromAPI(searchParams);
  ...
  const handleGroceryDownload = (dateRange) => {
    const groceryList = extractGroceryList(mealPlan, dateRange);
              ...
              <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
                ...
                {availableCategories.slice(0, 8).map(category => (
                  <button
                    ...
                    onClick={() => setActiveCategory(category.strCategory || category)}
                  >
